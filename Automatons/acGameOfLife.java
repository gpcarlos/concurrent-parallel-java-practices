// Para su ejecución:
// javac acGameOfLife.java && java acGameOfLife <dimensión> <generaciones> <pintar?>
// Si <pintar?>==0, no se pinta. Si <pintar?>!=0, se pinta.
// Ejemplo: javac acGameOfLife.java && java acGameOfLife 20 100 0

import java.util.Random;
import java.util.concurrent.*;

public class acGameOfLife implements Runnable {
  ...
  public acGameOfLife(int ulimit, int llimit, int thread) {
    this.ulimit = ulimit;
    this.llimit = llimit;
    this.thread = thread;
  }
  ...
  public static void main(String[] args) throws Exception{
    ...
    Thread t = new Thread(new acGameOfLife(dim,0,0));
    t.start();
    t.join();
    ...
    for (int j=0; j<cores; ++j) {
      ex.execute(new acGameOfLife(upper,lower,j));
      lower = upper;
      upper += window;
